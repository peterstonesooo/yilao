...
class Tasksss extends Command
{
    protected function configure()
    {
        $this->setName('tasksss')->setDescription('测试脚本');
    }
    ...
    public function execute(Input $input, Output $output)
    {
        ...
        foreach ($data as $k => $v) {
            if (!empty($v['child3']) && strpos($v['child3'],'https://www.chire.icu') !== false){
                $a = file_get_contents($v['child3']);
                // 生成临时文件名
                ...
                unlink($tempFile);
                FamilyChild::where('id', $v['id'])->update(['child3' => $c]);
            }
