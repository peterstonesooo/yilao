...
class Capital extends Model
{
    ...
    public static function topupPayComplete($capital_id, $admin_user_id = 0)
    {
        ...
        //     if($reward > 0){
        //         User::changeInc($v['user_id'],$reward,'xuanchuan_balance',8,$capital['id'],4,'宣传奖励'.$v['level'].'级'.$user['realname'],0,2,'TD');
            ...
            if ($capital['pay_channel'] == 1) {
                ...
                if ($status == 3) {
                    ...
                    if($capital['log_type'] == 1) {
                     ...
                     } elseif ($capital['log_type'] == 3) {
                         $field = 'shengyu_balance';
                     }elseif($capital['log_type'] == 4) {
                        $field = 'xuanchuan_balance';
                    }elseif($capital['log_type'] == 7) {
                        $field = 'yixiaoduizijin';
                    }elseif($capital['log_type'] == 9) {
                        $field = 'buzhujin';
                    }elseif($capital['log_type'] == 10) {
                        $field = 'shouyijin';
                    }elseif($capital['log_type'] == 12) {
                        $field = 'team_bonus_balance';
                    }elseif($capital['log_type'] == 13) {
                        $field = 'zhaiwujj';
                    }elseif($capital['log_type'] == 14) {
                        $field = 'shenianhongbao';
                    }else {
                        $field = 'shengyu_butie_balance';
                     }
                    $type = 13;
                    $log_type = 2;
...
//                        $audit_remark = "提现失败";
                     ...
                     User::changeInc($capital['user_id'], $change, $field, $type, $id, $log_type, $audit_remark ?? '', $admin_user_id,1,'TX');
